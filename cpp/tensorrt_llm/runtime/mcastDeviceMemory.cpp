...
void runCollectivePhase(McastGroupComm& comm, char const* phase, Action action, bool failureIsTerminal = true)
{
    ...
    int32_t const allSucceeded{comm.allreduceInt32Min(localSuccess)};
    if (allSucceeded != 0)
    {
        return;
    }
    ...
    , mNvlsHandle(nullptr)
{
    ...
    // Ordered membership, retained so a post-restore communicator can be validated against it.
    mGroupWorldRanks = mGroupComm->getWorldRanks();
    runCollectivePhase(*mGroupComm, "device capability validation",
        [this]()
        {
            TLLM_CHECK_WITH_INFO(
                mGroupRank < mGroupSize, "[McastDeviceMemory] Constructor group rank is out of range.");
...
void McastDeviceMemory::checkpointPrepare()
{
    ...
    int32_t const minState{mGroupComm->allreduceInt32Min(localState)};
    int32_t const maxState{mGroupComm->allreduceInt32Max(localState)};
...
bool McastDeviceMemory::checkpointRestore(int64_t mpiCommFortranHandle)
{
    TLLM_CHECK_WITH_INFO(
        mIsMNNvlink, "[McastDeviceMemory] Stable-VA checkpointing is only supported for fabric-backed MNNVL memory.");
    // Restore is MPI-only: the post-restore communicator arrives as an mpi4py Fortran handle.
    ...
    runCollectivePhase(
        *restoredGroupComm, "restore communicator validation",
        ...
        false);
    int32_t const localState{static_cast<int32_t>(mState)};
    int32_t const minState{restoredGroupComm->allreduceInt32Min(localState)};
    int32_t const maxState{restoredGroupComm->allreduceInt32Max(localState)};
    if (minState == static_cast<int32_t>(State::kMapped) && maxState == minState)
    {
        return false;
    }
