...
public:
    ...
    {
        return doAllreduceInt32(value, c10d::ReduceOp::MIN);
    }

    [[nodiscard]] int32_t allreduceInt32Max(int32_t value) override
    {
        return doAllreduceInt32(value, c10d::ReduceOp::MAX);
    }
...
private:
    ...
    //! Reduces in place on a host view of \p value over the group's CPU backend.
    int32_t doAllreduceInt32(int32_t value, c10d::ReduceOp::RedOpType op)
    {
        c10d::AllreduceOptions options;
        options.reduceOp = c10d::ReduceOp(op);
        std::vector<at::Tensor> payloads{at::from_blob(&value, {1}, at::TensorOptions{}.dtype(torch::kInt))};
        PGCHECK_THROW(mPg->allreduce(payloads, options));
        return value;
    }
