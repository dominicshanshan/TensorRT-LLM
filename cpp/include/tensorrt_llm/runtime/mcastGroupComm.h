...
public:
    ...
    //! Allreduce one int32 across the group with MIN / MAX. Used for the collective failure
    //! convergence and the checkpoint/restore state votes of McastDeviceMemory.
    [[nodiscard]] virtual int32_t allreduceInt32Min(int32_t value) = 0;
    [[nodiscard]] virtual int32_t allreduceInt32Max(int32_t value) = 0;
...
public:
    ...
    //! \param comm Group communicator; whether it is freed on destruction follows the MpiComm's own flag.
    explicit MpiMcastGroupComm(tensorrt_llm::mpi::MpiComm comm);
    ...
    [[nodiscard]] int32_t allreduceInt32Min(int32_t value) override;
    [[nodiscard]] int32_t allreduceInt32Max(int32_t value) override;
    ...
    //! The wrapped MPI communicator, for MPI-only lifecycle steps such as communicator duplication.
    [[nodiscard]] tensorrt_llm::mpi::MpiComm& getMpiComm()
    {
        return mComm;
    }
