...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Submit aggregated benchmark job')
    ...
    parser.add_argument('--dry-run',
                        action='store_true',
                        help='Dry run, test purpose only')
...
def submit_job(config, log_dir, dry_run):
    ...
    if extra_llm_api_path:
        server_cmd_parts.append(
            f"--extra_llm_api_options {extra_llm_api_path}")
    if ep_size > 1:
    ...
    streaming_flag = "" if benchmark_config.get('streaming',
                                                True) else "--non-streaming"
    ...
    cmd = [
        ...
        f'--ntasks={world_size}',
        f'--ntasks-per-node={gpus_per_node}',
        *([] if not slurm_config['set_segment'] else
        ...
        *([arg for arg in slurm_config['extra_args'].split() if arg]),
        ...
        '--cuda-architectures', env_config.get('cuda_architectures', ''),
        '--trtllm-wheel-path', env_config.get('trtllm_wheel_path', ''),
    ]
